...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 800, 800), 'pool_size': None,
...
default_cfgs = {
    'gnn_patch16_224': _cfg(
        crop_pct=0.9, input_size=(3, 800, 800),
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, channels, 50, 50))
    ...
    def forward(self, inputs, x=None):
        ...
        # print(li)
        # print("dim", self.prediction(x).squeeze(-1).squeeze(-1).shape)
        return x
